...
class Net(torch.nn.Module):
    def __init__(self, hidden_size, num_node_features, num_classes, num_conv_layers=3, num_linear_layers=1, dropout=0.5,
                 conv_layer='GCN', skip_connections=False, batch_norm=True, dummy_size=0, graph_level=False, device=None):
    ...
    def forward(self, data=None, x=None, edge_index=None, batch=None):
        try:
            ...
            x, edge_index, batch = data, x, edge_index
...
def train_model(model, train_loader, test_loader, optimizer=None, n_epochs=100, early_stop_patience=20,
                model_path=None, metrics=None, device='cpu'):
    if not optimizer:
    ...
    min_loss = np.inf
    best_epoch = -1
    outer_loop = tqdm(range(1, n_epochs + 1), leave=False, disable=False)
    
    for epoch in outer_loop:
        model.train()
        inner_loop = tqdm(train_loader, leave=False, disable=True)
        for data in inner_loop:
            data = data.to(device)
            optimizer.zero_grad()
            out = model(data)

            if model.graph_level:
        ...
        with torch.no_grad():
            for data in test_loader:
                ...
                outputs.append(out)
                if model.graph_level:
                    loss += F.nll_loss(out, data.y) * data.num_graphs
                else:
